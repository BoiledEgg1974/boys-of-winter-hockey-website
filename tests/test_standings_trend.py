...
class StandingsTrendTemplateTest(unittest.TestCase):
    def test_standings_template_markers(self) -> None:
        ...
        idx_link = template.index("standings-team-stats-link")
        self.assertLess(idx_chart, idx_link)
        self.assertNotIn('team_stats_panel("TEAM STATISTICS (REGULAR SEASON)"', template)
