...
**Sim log (`#sim-log`):** After admin **EXPORT** in the AP ledger, the site enqueues a **closed** export recap (`sim_cycle_update`) from GM export attendance for that date. The bot posts or edits the embed in `#sim-log`. There is no live in-progress board.
