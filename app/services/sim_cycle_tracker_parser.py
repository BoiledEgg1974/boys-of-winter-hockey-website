...
def parse_export_fhm_team_ids_from_messages(
    ...
    *,
    ...
    known_abbrevs = {abbr.upper() for _tid, (abbr, _em) in teams_for_league_slug(slug).items()}
    ...
    for message in messages:
        ...
        if cycle_started_at is not None:
            ...
            if ts is not None and ts.replace(tzinfo=None) < cycle_started_at.replace(tzinfo=None):
                continue

        for chunk in _collect_text_chunks(message):
            chunk_ids = _fhm_ids_from_text(slug, chunk)
            if chunk_ids:
                exported.update(chunk_ids)
                continue
            upper = chunk.upper()
            for abbr in known_abbrevs:
                if re.search(rf"\b{re.escape(abbr)}\b", upper):
                    tid = fhm_team_id_for_abbrev(slug, abbr)
                    if tid is not None:
                        exported.add(int(tid))
