...
**Playoff bracket:** Queued when [`playoff_bracket_cache_fingerprint`](app/services/playoff_bracket.py) changes (playoff game results or projected bracket from standings). Triggers: **CSV import** (`refresh_after_import`) and **bot poll** (same ~60s cadence as BOWL Six refresh). Map route **`playoff_bracket_update`** → `#playoff-bracket` on each league’s Discord Integration page. The bot posts one message per series; on updates it **PATCH**es each series message using stored message IDs (same pattern as BOWL Six, but multi-message). Manual **`/predict`** remains separate for pre-playoff **prediction** posts in `#playoff-predictions`.
