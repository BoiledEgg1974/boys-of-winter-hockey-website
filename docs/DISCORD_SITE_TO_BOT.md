...
**Playoff predictions (`/predict`):** Admin-only slash command in `#playoff-predictions`. Run `/predict` with no options to list rounds that still need predictions (completed series are skipped). Queue a round with `/predict round:second` (aliases: `1st`, `2nd`, `conference`, `championship`, `all`). Re-register slash commands after deploy: `python -m scripts.league_discord_bot.register_slash_commands`.
