...
def ensure_discord_outbound_sqlite(engine: Engine) -> None:
    ...
    with engine.connect() as conn:
        ...
        has_team_routes = conn.execute(
            text(
                "SELECT 1 FROM sqlite_master WHERE type='table' "
                "AND name='discord_team_channel_routes'"
            )
        ).fetchone()
        if not has_team_routes:
            conn.execute(
                text(
                    """
                    CREATE TABLE discord_team_channel_routes (
                        id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
                        league_slug VARCHAR(64) NOT NULL,
                        event_key VARCHAR(64) NOT NULL,
                        team_id INTEGER NOT NULL,
            ...
            conn.execute(
                text(
                    "CREATE UNIQUE INDEX uq_discord_team_route_league_event_team "
                    "ON discord_team_channel_routes (league_slug, event_key, team_id)"
                )
            )
            conn.execute(
                text(
                    "CREATE INDEX ix_discord_team_route_league_event "
