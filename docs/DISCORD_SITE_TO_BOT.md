...
**Sim log (`#sim-log`):** **Live** progress is **Discord-only**: `league_discord_bot` polls **`#gm-export-tracker`** every ~3s (route **`gm_export_tracker_poll`**) and parses export-bot posts (team emoji / abbrev + “exported”). It **PATCH**es the live embed in **`#sim-log`** (route **`sim_cycle_update`**). The website does not increment live counts. **Closed** recap (after admin **EXPORT**) uses GM export attendance on the site for that date, then starts a fresh live cycle at 0 for the next sim. Configure **both** routes on every league server before expecting auto-updates.
